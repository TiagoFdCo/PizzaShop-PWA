...
export async function getProducts(): Promise<Pizza[]> {
  return apiFetch<Pizza[]>(ENDPOINT, { useMockApi: true });
}

export async function getProductById(id: string): Promise<Pizza> {
  return apiFetch<Pizza>(`${ENDPOINT}/${id}`, { useMockApi: true });
}
...
export async function deleteProduct(id: string): Promise<void> {
  await apiFetch<void>(`${ENDPOINT}/${id}`, { method: "DELETE", useMockApi: true });
}