"""Models do PRESENCIAL: mesa (restaurant_table) e comanda (tab).

Uma mesa tem MUITAS comandas (1:N via Tab.table_id). Uma comanda agrupa
vários pedidos (Tab.orders <-> Order.tab).
"""
import enum
import uuid
...
class RestaurantTable(Base):
    """Uma mesa do salao. 1 mesa : N comandas (RestaurantTable.tabs)."""
...
class Tab(Base):
    """Comanda. Pertence a UMA mesa (table_id NOT NULL) e agrupa varios pedidos."""
    ...
    def __repr__(self) -> str:
        return f"<Tab id={self.id} table={self.table_id} status={self.status}>"
